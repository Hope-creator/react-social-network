...
router.get("/photos/:userId", async (req, res) => {
    ...
    try {
        ...
        if (!profilePhotos) res.json({ success: false, error: "Photos not finded" })
        else {
...
// #access: Private
router.post("/photos", authenticateToken, createUserFolderPhotos, uploadPhotos.array('profilePhotos'), async (req, res) => {
    const id = req.userId;
    const { ts } = req.body;
    const errors = [];
...
router.put("/status", authenticateToken, async (req, res) => {
    ...
    if (newStatus.length > 300) res.json({ success: false, error: "Status is too long" })
    else {
        try {
            const user = await User.findByIdAndUpdate(id,
...
router.put("/", authenticateToken, async (req, res) => {
    ...
    if (aboutMe && aboutMe.length > 300) errors.push("About field is 300 symbols max");
    if (typeof lookingForAJob !== "boolean") errors.push("Wrong looking for a job type")
    if (lookingForAJobDescription && lookingForAJobDescription.length > 300) errors.push("Looking for a job description is 300 symbols max");
