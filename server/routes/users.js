...
router.get("/", async (req, res) => {
    ...
    const friendRequest = friend ? { followers: { '$elemMatch': { $eq: friend } } } : {};
    ...
    try {
        if (random) {
            ...
            if (!randomFriends) res.json({success: false, error: "Something went's wrong"})
            else {
                res.json({success: true, randomFriends})
            }
        }
        else {
            const users = await User.find({ $and: [query, friendRequest] }, "name id profile.status profile.profilePicture followers")
                .skip(pageQuery)
                .limit(+count)
            const totalCount = await User.find({ $and: [query, friendRequest] }).countDocuments()
