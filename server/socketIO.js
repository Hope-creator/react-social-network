...
module.exports = function(io) {
    io.on('connection', async (socket) => {
        ...
        try {
            ...
            if (unreadConversations) socket.emit("unreadConversations", unreadConversations)
        }
        catch (err) {
            ...
            try {
                ...
                if (readConversation) {
                    try {
                        ...
                        console.log("ERROR ON SEND UNREAD CONVERSATION AFTER READING:", err)
                    }
                }
            }
            catch (err) {
        ...
        socket.on("message", async ({ content, to, context }, callback) => {
            ...
            try {
                ...
                if (isConversationExist) {
                    ...
                    if (conversationFrom && conversationTo && savedMessage && unreadConversationsTo) {
                        socket.to(peerId).emit("message", {
                            message: savedMessage
                        });
                        socket.to(peerId).emit("updateConversation", conversationTo)
                        socket.to(peerId).emit("unreadConversations", unreadConversationsTo)
                        if (context === 0) {
                            callback({
                                success: true
                            })
