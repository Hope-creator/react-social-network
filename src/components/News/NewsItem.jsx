...
const NewsItem = ({id, type, profilePic, profile, propsDate, text, attachments }) => {
    ...
    return <>
        <div className={s.newsItemPost}>
            ...
            <div className={s.newsitemContent}>
                ...
                item.type == "photo" ?
                <img id={item.id} onClick={()=> {changeFullScreen();settingFullScreenImgSrc(item.url)}}  className={array.length == 1 ? s.newsPhotoObject : s.newsSmallObject} src={item.url} alt={`photo__${item.id}`}  /> : 
                item.type == "video" ?
