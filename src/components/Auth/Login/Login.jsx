...
const LoginFormWrap = (props) => {
    ...
    const onSubmit = async values => {
        ...
        await props.authLoginThunk(values.login, values.password,values.captcha)
            .then(response => {
                if(response && response.errors) response.errors.forEach(error=>errors.push(error.msg))
            })
        ...
        <Form
            ...
            render={({ handleSubmit, submitError }) => (
                <form onSubmit={handleSubmit}>
                    ...
                    <div>
                        <span  className={s.footerText}><NavLink to="/join">Create account</NavLink></span>
                    </div>
                    <button className={s.submitBtn} type="submit"> <span className={s.submitBtnText}>Login</span></button>
                    <div>
                        <span className={s.footerText}><NavLink to="/reset">Forgot password?</NavLink></span>
                    </div>
                </form>
            )}
        />
